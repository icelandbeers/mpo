...
class Userbot(Client):
    ...
    async def start(self):
        LOGGER(__name__).info(f"MEMULAI ASISSTANT CLIENTS")
        if config.STRING1:
            await self.one.start()
            try:
                await self.five.join_chat("DantePedia")
                await self.five.join_chat("dansupport")
            except:
                pass
            assistants.append(1)
            try:
                await self.one.send_message(
                    config.LOG_GROUP_ID, "ASISSTANT AKTIF KONTOL"
                )
            except:
            ...
            LOGGER(__name__).info(
                f"ASISSTANT SATU DIMULAI SEBAGAI {self.one.name}"
            )
        if config.STRING2:
            ...
            LOGGER(__name__).info(
                f"ASISSTANT DUA DIMULAI SEBAGAI {self.two.name}"
            )
        if config.STRING3:
            ...
            LOGGER(__name__).info(
                f"ASISSTANT TIGA DIMULAI SEBAGAI {self.three.name}"
            )
        if config.STRING4:
            ...
            LOGGER(__name__).info(
                f"ASISSTANT EMPAT DIMULAI SEBAGAI {self.four.name}"
            )
        if config.STRING5:
            ...
            LOGGER(__name__).info(
                f"ASISSTANT LIMA DIMULAI SEBAGAI {self.five.name}"
            )